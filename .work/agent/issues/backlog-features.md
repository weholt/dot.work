# Backlog: Features (Skills & Subagents)

Feature requests for Agent Skills and Cross-environment Subagent support.

Issues: FEAT-023, FEAT-024
...
### Prompt Migration Analysis (2026-01-01)

Analysis of `src/dot_work/prompts/` identified prompts suitable for migration to skills:

**Tier 1: Strong Skill Candidates (migrate first)**

| Prompt | Skill Name | Trigger | Lines |
|--------|------------|---------|-------|
| `establish-baseline.md` | `establish-baseline` | "generate baseline", start of iteration | 331 |
| `compare-baseline.md` | `compare-baseline` | "validate", "check regressions" | 464 |
| `critical-code-review.md` | `code-review` | "review code", PR review | 368 |
| `spec-delivery-auditor.md` | `spec-audit` | "verify implementation" | 393 |
| `new-issue.md` | `create-issue` | "create issue", "log bug" | 222 |
| `bump-version.md` | `bump-version` | "bump version", "release" | 481 |
| `api-export.md` | `api-audit` | "review API", "audit endpoints" | 362 |
| `housekeeping.md` | `housekeeping` | "cleanup", "housekeeping" | 34 |

**Tier 2: Good Skill Candidates (migrate after Tier 1)**

| Prompt | Skill Name | Trigger | Lines |
|--------|------------|---------|-------|
| `security-review.md` | `security-review` | "security audit" | 109 |
| `performance-review.md` | `performance-review` | "performance check" | 119 |
| `pythonic-code.md` | `make-pythonic` | "refactor python" | 90 |
| `task-assessment.md` | `assess-task` | "evaluate task" | 113 |
| `improvement-discovery.md` | `discover-improvements` | "find improvements" | 227 |
| `external-project-reality-auditor.md` | `reality-audit` | "audit project" | 240 |
| `create-constitution.md` | `create-constitution` | "create constitution" | 25 |
| `python-project-from-discussion.md` | `scaffold-python` | "create python project" | 990 |

**NOT Suitable for Skills (keep as prompts)**

| Prompt | Reason |
|--------|--------|
| `do-work.md` | Master orchestrator, requires persistent state (1256 lines) |
| `setup-issue-tracker.md` | One-time setup with filesystem changes |
| `issue-readiness.md` | Interactive, requires user back-and-forth |
| `eat-our-own-dogfood.md` | Multi-phase with human approval gates |
| `agent-prompts-reference.md` | Pure documentation |
| `code-review.md` | General checklist (less structured than critical-code-review) |
| `production-ready-apis.md` | Reference checklist |
...
description: >
  Captures a frozen, evidence-backed snapshot of project state for regression
  detection. Produces .work/baseline.md with 8 axes of analysis.
license: MIT
compatibility: "Requires access to project files, git, and test output"
metadata:
  author: dot-work
  version: "1.0"
  category: audit
...
**Migration Path:**
1. Implement skills infrastructure (this issue)
2. Migrate Tier 1 prompts (8 high-value skills)
3. Keep workflow prompts as prompts
4. Migrate Tier 2 based on usage patterns
