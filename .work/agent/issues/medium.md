...
description: "Only 1 of 8 prompts uses template variables for paths"
created: 2024-12-20
section: "prompts"
tags: [prompts, templates, consistency]
type: enhancement
priority: medium
status: proposed
references:
  - src/dot_work/prompts/do-work.prompt.md
  - src/dot_work/prompts/critical-code-review.prompt.md
  - src/dot_work/prompts/establish-baseline.prompt.md
  - src/dot_work/prompts/compare-baseline.prompt.md
  - src/dot_work/prompts/spec-delivery-auditor.prompt.md
  - src/dot_work/prompts/agent-prompts-reference.prompt.md
---

### Problem
Prompts use hardcoded relative paths like `[do-work.prompt.md](do-work.prompt.md)` instead of template variables like `[do-work.prompt.md]({{ prompt_path }}/do-work.prompt.md)`. This works for same-directory installs but produces incorrect links for environments with different prompt locations.

### Affected Files
- All `.prompt.md` files in `src/dot_work/prompts/` except `setup-issue-tracker.prompt.md` (already uses templates)

### Importance
Links will be broken when installed to environments like Claude (CLAUDE.md) or Cursor (.cursor/rules/). Users following cross-references will hit dead links.

### Proposed Solution
1. Audit all prompt files for cross-references
2. Replace hardcoded paths with `{{ prompt_path }}/filename.prompt.md`
3. Update tests to verify no hardcoded `.prompt.md` references remain
...
### Notes
The Related Documentation sections in each prompt need updating.
