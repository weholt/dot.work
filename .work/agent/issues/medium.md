...
title: "Add tests for container provision module"
description: "Create unit tests for container provisioning functionality"
created: 2024-12-21
section: "tests"
tags: [migration, container, tests]
type: test
priority: medium
status: proposed
references:
...
### Problem
The container provision module needs tests.
...
**test_validation.py:**
- `test_validate_valid_instructions`
- `test_validate_missing_repo_url`
- `test_validate_invalid_frontmatter`

**test_templates.py:**
- `test_default_template_is_valid`
...
### Acceptance Criteria
- [ ] Tests in `tests/unit/container/provision/`
- [ ] Coverage ≥ 80%
- [ ] All tests pass
- [ ] Mock Docker/Git operations

### Notes
Depends on MIGRATE-048 (module must be functional).
...
title: "Create overview module structure in dot-work"
description: "Create src/dot_work/overview/ module from birdseye project"
created: 2024-12-21
section: "overview"
tags: [migration, birdseye, overview, analysis]
type: enhancement
priority: medium
status: proposed
references:
  - incoming/crampus/birdseye/src/birdseye/
  - src/dot_work/overview/
---

### Problem
The birdseye project scans a project directory to produce a concise overview markdown and structured JSON datasets. Integrate as `dot-work overview`.

### Source Files to Migrate
From `incoming/crampus/birdseye/src/birdseye/`:
- `cli.py` - Typer CLI
- `pipeline.py` - Main analysis pipeline
- `scanner.py` - File scanning
- `code_parser.py` - Python code parsing (libcst)
...
### Proposed Solution
1. Create `src/dot_work/overview/` directory
2. Copy all source files
3. Adapt cli.py for dot-work registration
4. Update imports
...
### Notes
Uses libcst for Python parsing and radon for metrics.
...
title: "Update overview module imports and config"
description: "Refactor imports from birdseye.* to dot_work.overview.*"
created: 2024-12-21
section: "overview"
tags: [migration, overview, imports, refactor]
type: refactor
priority: medium
status: proposed
references:
...
### Problem
After copying files, imports reference `birdseye.*` which doesn't exist.

### Import Changes Required

| Old Import | New Import |
|------------|------------|
| `from birdseye.pipeline import analyze_project` | `from dot_work.overview.pipeline import analyze_project` |
| `from birdseye.scanner import scan_files` | `from dot_work.overview.scanner import scan_files` |
| `from birdseye.models import Bundle` | `from dot_work.overview.models import Bundle` |

### Proposed Solution
1. Update all internal imports
2. Verify: `uv run python -c "from dot_work.overview import analyze_project"`
...
### Notes
Depends on MIGRATE-058 (files must exist first).
...
title: "Register overview as top-level subcommand in dot-work CLI"
description: "Add overview command as 'dot-work overview <input> <output>'"
created: 2024-12-21
section: "cli"
tags: [migration, overview, cli, integration]
type: enhancement
priority: medium
status: proposed
references:
...
### Problem
The overview module needs CLI integration as top-level command.

### CLI Design
```bash
# Analyze project and generate overview
dot-work overview <input-dir> <output-dir>

# Examples
dot-work overview . ./docs/overview
dot-work overview src/ reports/
```

### Output Files
```
<output-dir>/
├── birdseye_overview.md   # Human-readable overview
├── features.json          # Structured features
└── documents.json         # Document summaries
```
...
def overview(
    input_dir: Path = typer.Argument(..., help="Project directory to scan"),
...
### Proposed Solution
1. Add `overview` command directly to main app (top-level)
2. Import from overview module
3. Generate three output files

### Acceptance Criteria
- [ ] `dot-work overview --help` shows usage
- [ ] `dot-work overview <input> <output>` works
- [ ] Generates overview.md, features.json, documents.json

### Notes
Depends on MIGRATE-059 (imports must work first).
...
title: "Add overview dependencies to pyproject.toml"
description: "Add libcst and radon for overview analysis"
created: 2024-12-21
section: "dependencies"
tags: [migration, overview, dependencies, pyproject]
type: enhancement
priority: medium
status: proposed
references:
...
### Problem
The overview module requires external dependencies.

### Dependencies from birdseye
```toml
dependencies = [
    "libcst>=1.1",    # Python code parsing
    "radon>=6.0",     # Metrics (also used by scan module)
    "typer>=0.12",    # Already in dot-work
]
```

### Proposed Solution
1. Add `libcst>=1.1` to core dependencies
2. `radon` may already be added for scan module - verify
3. Run `uv sync` to install
4. Verify imports work

### Acceptance Criteria
- [ ] `libcst` in core dependencies
- [ ] `radon` present (may already exist from scan)
- [ ] `uv sync` succeeds
- [ ] Overview module imports work

### Notes
Check if radon already added for MIGRATE-030 (scan module).

Depends on MIGRATE-058 (module must exist).
...
title: "Add tests for overview module"
description: "Create unit tests for overview analysis functionality"
created: 2024-12-21
section: "tests"
tags: [migration, overview, tests]
type: test
priority: medium
status: proposed
references:
...
### Problem
The overview module needs tests.

### Test Structure
```
tests/unit/overview/
├── __init__.py
├── conftest.py          # Fixtures (sample projects)
├── test_pipeline.py     # Pipeline tests
├── test_scanner.py      # Scanner tests
├── test_code_parser.py  # Code parser tests
...
**test_pipeline.py:**
- `test_analyze_project_returns_bundle`
- `test_write_outputs_creates_files`

**test_scanner.py:**
- `test_scan_finds_python_files`
- `test_scan_finds_markdown_files`

**test_code_parser.py:**
- `test_parse_extracts_classes`
- `test_parse_extracts_functions`

### Acceptance Criteria
- [ ] Tests in `tests/unit/overview/`
- [ ] Coverage ≥ 80%
- [ ] All tests pass
- [ ] Use sample project fixtures

### Notes
Depends on MIGRATE-059 (module must be functional).
...
title: "Verify overview migration with full build"
description: "Run complete build pipeline and verify overview functionality"
created: 2024-12-21
section: "overview"
tags: [migration, overview, verification, qa]
type: test
priority: medium
status: proposed
references:
...
### Problem
After completing all migration steps, verify the overview migration works correctly.
...
**CLI Verification:**
```bash
# Analyze dot-work itself
dot-work overview src/ .work/overview-test/

# Check outputs
cat .work/overview-test/birdseye_overview.md | head -50
cat .work/overview-test/features.json | head -20
cat .work/overview-test/documents.json | head -20

# Cleanup
rm -rf .work/overview-test/
```
...
- [ ] Output contains meaningful analysis

### Notes
Final verification. Depends on: MIGRATE-058 through MIGRATE-062.
