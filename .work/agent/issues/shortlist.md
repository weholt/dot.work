...
created: 2025-12-26
section: "skills"
tags: [feature, agent-skills, discovery, prompts, progressive-disclosure, has-deps]
type: enhancement
priority: medium
status: proposed
references:
...
### Importance
Skills provide progressive disclosure and reusable capabilities. Complements existing prompts feature with dynamic runtime injection.

### Proposed Solution
Follow skills_spec.md reference:
...
### Dependencies
Blocked by: None
Blocks: FEAT-024 (subagents may use skills)

### Clarifications Needed
None. Reference spec in `.work/agent/issues/references/skills_spec.md` is complete.

### Notes
See `.work/agent/issues/references/skills_spec.md` for full specification including validation rules, CLI commands, and test structure.
...
created: 2025-12-26
section: "subagents"
tags: [feature, subagents, custom-agents, claude-code, opencode, copilot, multi-environment, has-deps]
type: enhancement
priority: medium
status: proposed
references:
...
### Problem
Users need to define specialized AI agents (subagents) that work across Claude Code, OpenCode, and GitHub Copilot. Each platform has different formats, and there is no unified way to manage subagent definitions.

### Affected Files
- Create: `src/dot_work/subagents/__init__.py`, `models.py`, `parser.py`, `validator.py`, `discovery.py`, `generator.py`
- Create: `src/dot_work/subagents/environments/__init__.py`, `base.py`, `claude_code.py`, `opencode.py`, `copilot.py`
- Create: `tests/unit/subagents/` and `tests/unit/subagents/environments/`
- Create: `tests/fixtures/subagents/canonical/` and `tests/fixtures/subagents/native/`

### Importance
Subagents enable specialized AI personalities with tool/permission configurations. Cross-platform support allows consistent agent behavior across environments.

### Proposed Solution
Follow subagents_spec.md reference:
1. Create data models (`SubagentMetadata`, `SubagentConfig`, `CanonicalSubagent`)
2. Implement markdown + YAML frontmatter parser (native and canonical formats)
3. Build environment adapter base class and three implementations (Claude Code, OpenCode, Copilot)
4. Implement canonical-to-native generator with tool/model name mapping
5. Add discovery for native and canonical subagents
6. Add CLI: `dot-work subagents list/validate/show/install/generate/sync`
7. Bundle 6 starter subagents (code-reviewer, test-runner, debugger, docs-writer, security-auditor, refactorer)

### Acceptance Criteria
- [ ] Parser extracts both canonical and native format subagents
- [ ] Environment adapters generate correct native files for each platform
- [ ] Tool name mapping matches specification table (Read→read/Read, etc.)
- [ ] CLI `list` shows discovered subagents
- [ ] CLI `generate` produces platform-specific file content
- [ ] CLI `install` writes to correct target directory
- [ ] Bundled subagents validate and generate correctly
- [ ] Unit tests for all modules (≥75% coverage)
- [ ] Integration tests for each environment adapter

### Validation Plan
```bash
# Unit tests
uv run pytest tests/unit/subagents/ -v --cov=src/dot_work/subagents --cov-report=term-missing

# CLI smoke test
uv run dot-work subagents list --env claude
uv run dot-work subagents generate code-reviewer.md --env opencode
uv run dot-work subagents install code-reviewer.md --env copilot

# Validate bundled subagents
uv run dot-work subagents validate .work/subagents/code-reviewer.md
```
...
### Clarifications Needed
None. Reference spec in `.work/agent/issues/references/subagents_spec.md` is complete.

### Notes
See `.work/agent/issues/references/subagents_spec.md` for full specification including platform comparison, tool mapping, and implementation phases.
...
references:
  ...
  - src/dot_work/container/provision/validation.py

### Problem
Users need isolated OpenCode environments in Docker containers. Current `src/dot_work/container/provision/` module exists but needs: dynamic port assignment (avoid conflicts), credential injection (without exposure in logs/processes), and optional repo cloning.

### Affected Files
- Modify: `src/dot_work/container/provision/core.py` (add dynamic port allocation, credential mount)
- Modify: `src/dot_work/container/provision/cli.py` (add port, credential, clone options; background mode)
- Modify: `src/dot_work/container/provision/validation.py` (add port range validation)
- Create: `tests/integration/container/test_provision.py`

### Importance
High priority. Docker isolation is critical for testing OpenCode configurations safely. Dynamic ports enable parallel containers.

### Proposed Solution
1. **Dynamic Port Assignment**: Add `find_available_port()` function using socket binding test
   - Default range: 8000-9000 (search for available port)
   - Range override via `.env` file: `PORT_RANGE_MIN` and `PORT_RANGE_MAX`
2. **Credential Injection**: Volume mount `~/.local/share/opencode/auth.json` into container
   - Mount to `/root/.local/share/opencode/auth.json` inside container
   - Not visible in `docker ps` environment (secure)
3. **GitHub Cloning**: Add `--clone REPO_URL` option with git clone inside container
4. **WebUI URL Output**: Print `http://localhost:<dynamic-port>` and container ID after start
5. **Container Lifecycle**: Always run in background mode, print container ID

### Acceptance Criteria
- [ ] CLI `provision` command assigns available port in 8000-9000 range by default
- [ ] Port range configurable via `.env` file (PORT_RANGE_MIN/MAX)
- [ ] Credentials passed via volume mount, not visible in `docker ps` environment
- [ ] `--clone REPO_URL` clones repo into container workspace
- [ ] Container runs in background, prints container ID and webUI URL
- [ ] Integration tests verify port availability and container startup
- [ ] Documentation updated with usage examples

### Validation Plan
```bash
# Unit tests for port finding
uv run pytest tests/unit/container/test_port_allocation.py -v

# Integration smoke test (requires Docker)
uv run dot-work container provision --image opencode:latest --clone https://github.com/user/repo

# Verify port is available and webUI accessible
curl -f http://localhost:<dynamic-port> || exit 1
```

### Dependencies
Blocked by: None
Blocks: FEAT-026, FEAT-027, FEAT-028

### Clarifications Needed
None. All decisions resolved:
- Credential handling: Volume mount `~/.local/share/opencode/auth.json` into container
- Port range: Dynamic 8000-9000 with `.env` override support
- Container lifecycle: Always background, print container ID

### Notes
Existing code in `src/dot_work/container/provision/` from repo-agent project. May need refactoring to remove repo-agent-specific assumptions (PR workflow, git user config).
