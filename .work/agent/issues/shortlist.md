...
### Affected Files
- `src/dot_work/` (new directories needed)
- `pyproject.toml` (package data configuration)

### Importance
Aligning skills/subagents with the prompts pattern enables:
- Consistent "batteries included" experience
- Single installation flow for all content types
- Shipped pre-defined skills/subagents that users can immediately use
...
### Acceptance Criteria
- [ ] `src/dot_work/bundled_skills/` directory created
- [ ] `src/dot_work/bundled_subagents/` directory created
- [ ] `global.yml` files created with proper structure
- [ ] `pyproject.toml` updated to include package data
- [ ] Build successfully includes the new directories
- [ ] `importlib.resources` can access the new directories
