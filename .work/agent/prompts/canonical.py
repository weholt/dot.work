...
def parse_canonical_prompt(file_path: str) -> CanonicalPrompt:
    ...
    if not environments:
        errors.append(ValidationError(
            line=1,
            message="Missing environments block"
        ))
    ...
    for env_name, env_config in environments.items():
        if 'target' not in env_config:
            errors.append(ValidationError(
                line=1,
                message=f"Environment '{env_name}' missing required 'target' field"
            ))
    ...
    for env_name, env_config in prompt.environments.items():
        ...
        if 'keys' in env_config:
            ...
            if not isinstance(keys, dict):
                errors.append(ValidationError(
                    line=1,
                    message=f"Environment '{env_name}' 'keys' must be a dictionary"
                ))
