"""Unit tests for agent_review.storage module."""
...
from agent_review.storage import (
    append_comment,
    ensure_store,
    latest_review_id,
    load_comments,
...
class TestEnsureStore:
    ...
    def test_creates_structure(self, temp_dir: Path) -> None:
        """Test that storage structure is created."""
        base = ensure_store(str(temp_dir))
        assert base.exists()
    ...
    def test_idempotent(self, temp_dir: Path) -> None:
        ...
        base1 = ensure_store(str(temp_dir))
        base2 = ensure_store(str(temp_dir))
        assert base1 == base2
...
class TestReviewDir:
    ...
    def test_creates_directory(self, temp_dir: Path) -> None:
        ...
        rd = review_dir(str(temp_dir), "test-review")
        assert rd.exists()
...
class TestAppendAndLoadComments:
    ...
    def test_append_and_load(self, temp_dir: Path) -> None:
        ...
        append_comment(str(temp_dir), comment)

        comments = load_comments(str(temp_dir), "test-review")
        assert len(comments) == 1
        assert comments[0].path == "src/test.py"
        assert comments[0].message == "Fix this"

    def test_multiple_comments(self, temp_dir: Path) -> None:
    ...
    def test_filter_by_path(self, temp_dir: Path) -> None:
        ...
        comments = load_comments(str(temp_dir), "test", path="file1.py")
        assert len(comments) == 1
        assert comments[0].path == "file1.py"

    def test_load_empty_review(self, temp_dir: Path) -> None:
        """Test loading from non-existent review."""
        comments = load_comments(str(temp_dir), "nonexistent")
        assert comments == []
...
class TestLatestReviewId:
    ...
    def test_no_reviews(self, temp_dir: Path) -> None:
        """Test with no reviews."""
        result = latest_review_id(str(temp_dir))
        assert result is None

    def test_single_review(self, temp_dir: Path) -> None:
        """Test with single review."""
        review_dir(str(temp_dir), "20241220-120000")
        result = latest_review_id(str(temp_dir))
        assert result == "20241220-120000"

    def test_multiple_reviews(self, temp_dir: Path) -> None:
        """Test with multiple reviews - returns latest."""
        review_dir(str(temp_dir), "20241219-100000")
        review_dir(str(temp_dir), "20241220-150000")
        review_dir(str(temp_dir), "20241220-120000")
