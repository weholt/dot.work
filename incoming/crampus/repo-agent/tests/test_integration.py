...
import pytest
from repo_agent.core import RepoAgentError, run_from_markdown
...
""")
    ...
    def test_model_override(self, basic_instructions_file):
        ...
        with patch("repo_agent.core.subprocess.run"):
            run_from_markdown(basic_instructions_file, model="gpt-4", dry_run=True)

    def test_branch_override(self, basic_instructions_file):
        """Test that branch can be overridden via CLI."""
        with patch("repo_agent.core.subprocess.run"):
            run_from_markdown(basic_instructions_file, branch="custom-branch", dry_run=True)
...
""")
    ...
    @patch("repo_agent.core.subprocess.run")
    def test_docker_command_includes_required_env_vars(
        self, mock_run, minimal_instructions_file, monkeypatch
    ):
