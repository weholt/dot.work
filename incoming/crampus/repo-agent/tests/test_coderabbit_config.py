...
class TestCodeRabbitConfig:
    ...
    def test_required_sections(self, config):
        ...
        required_sections = ['language', 'reviews', 'finishing_touches', 'pre_merge_checks', 'tools']
    ...
    def test_auto_review_enabled(self, config):
        ...
        assert auto_review['auto_incremental_review'] is True, "Auto incremental review should be enabled"
    ...
    def test_high_level_summary(self, config):
        ...
        assert 'high_level_summary_instructions' in reviews, "Summary instructions should be present"
    ...
    def test_path_instructions(self, config):
        ...
        assert 'dataclasses' in python_path['instructions'], "Python instructions should mention dataclasses"
    ...
    def test_finishing_touches(self, config):
        ...
        assert finishing['docstrings']['enabled'] is True, "Docstrings finishing touch should be enabled"
        assert finishing['unit_tests']['enabled'] is True, "Unit tests finishing touch should be enabled"
    ...
    def test_code_generation(self, config):
        ...
        assert len(code_gen['docstrings']['path_instructions']) > 0, "Docstrings path instructions should exist"
        
        # Check unit tests configuration
        assert len(code_gen['unit_tests']['path_instructions']) > 0, "Unit tests path instructions should exist"
    ...
    def test_pr_title_requirements(self, config):
        ...
        assert '<area>: <action>' in reviews['auto_title_instructions'], "Title format should be specified"
        
        pre_merge = config['pre_merge_checks']
        assert 'title' in pre_merge, "Title check should be configured"
        assert '<area>: <action>' in pre_merge['title']['requirements'], "Title requirements should match format"
