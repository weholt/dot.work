...
class _Collector(cst.CSTVisitor):
    ...
    def visit_ClassDef(self, node: cst.ClassDef) -> None:
        ...
        bases = [
            expr for expr in (self._render_expression(getattr(base, "value", None)) for base in node.bases) if expr
        ]
    ...
    def visit_FunctionDef(self, node: cst.FunctionDef) -> None:
        args = [
            Argument(
                ...
                annotation=self._render_expression(param.annotation.annotation) if param.annotation else None,
            )
    ...
    def _extract_fields(self, node: cst.ClassDef) -> Iterable[ModelField]:
        for element in node.body.body:
            ...
            if isinstance(element, cst.AnnAssign):
                ...
                ann_assignments.extend(stmt for stmt in element.body if isinstance(stmt, cst.AnnAssign))
            ...
            for ann in ann_assignments:
                ...
                yield ModelField(
                    name=field_name,
                    annotation=self._render_expression(annotation_node) if annotation_node else None,
    ...
    def _render_expression(self, node: cst.CSTNode | None) -> str | None:
        if node is None:
            return None
        try:
            return _MODULE_TEMPLATE.code_for_node(node)
        except Exception:
            return None

    def _interface_flags(self, decorators: Iterable[str | None], docstring: str | None) -> dict[str, bool]:
    ...
    def _is_user_facing(self, feature: Feature) -> bool:
        ...
        if any(getattr(feature, key) for key in ("api", "cli", "web", "mcp")):
            return True
        if any(token in name for token in ("handle", "serve", "command", "endpoint", "route", "process_request")):
            return True
        return any(token in doc for token in ("user", "input", "request", "api", "cli", "browser", "http"))

    # ---------------------------------------------------------------------- #


def export_features_to_json(features: Iterable[Feature], models: Iterable[ModelDef], destination: Path) -> None:
