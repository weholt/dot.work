...
class PlannerAgent(BaseAgent):
    ...
    async def execute(
        ...
        try:
            ...
            result = self._create_validation_result(
                subtask_id=task.id,
                passed=False,
                issues=[f"Task decomposition failed: {str(e)}"]
            )
            return result
    ...
    def _extract_subtask_description(self, lines: List[str], current_index: int) -> str:
        ...
        for i in range(current_index + 1, min(current_index + 3, len(lines))):
            ...
            if next_line and not next_line.startswith('#') and not re.match(r'^(?:add|implement|create|fix|test|document)', next_line, re.IGNORECASE):
