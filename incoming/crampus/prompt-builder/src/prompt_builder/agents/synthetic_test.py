...
class SyntheticTestAgent(BaseAgent):
        ...
        try:
            if not subtask:
                return self._create_validation_result(
                    subtask_id=task.id, passed=False, issues=["Synthetic test generation requires a specific subtask"]
                )
            ...
            test_generation_result = await self._generate_synthetic_tests(subtask, coverage_analysis["uncovered_paths"])
            ...
            test_execution_result = await self._run_synthetic_tests(test_generation_result["generated_tests"])
    ...
    async def _analyze_coverage(self, subtask: Subtask) -> dict[str, Any]:
        ...
        uncovered_paths = []
        metrics = {"files_analyzed": 0, "functions_found": 0, "uncovered_branches": 0, "coverage_percentage": 0.0}
        ...
        try:
            ...
            if total_branches > 0:
                metrics["coverage_percentage"] = (total_branches - metrics["uncovered_branches"]) / total_branches * 100

        except Exception as e:
            self.logger.warning(f"Coverage analysis failed: {e}")

        return {"uncovered_paths": uncovered_paths, "metrics": metrics}

    async def _generate_synthetic_tests(
        self, subtask: Subtask, uncovered_paths: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """Generate synthetic tests for uncovered code paths."""
        generated_tests = []
        issues = []
        warnings = []
        metrics = {"tests_generated": 0, "edge_case_tests": 0, "boundary_value_tests": 0}
        ...
        return {"generated_tests": generated_tests, "issues": issues, "warnings": warnings, "metrics": metrics}
    ...
    async def _run_synthetic_tests(self, test_files: list[str]) -> dict[str, Any]:
        ...
        if not test_files:
            return {
                "issues": [],
                "warnings": [],
                "metrics": {"synthetic_tests_run": 0, "synthetic_tests_passed": 0, "synthetic_tests_failed": 0},
            }
        ...
        try:
            ...
            cmd = ["python", "-m", "pytest"] + test_files + ["--tb=short", "-v", "--no-header"]
    ...
    async def _analyze_file_coverage(self, file_path: str, coverage_data: dict[str, Any]) -> dict[str, Any]:
        ...
        analysis = {"functions": 0, "uncovered_branches": 0, "uncovered_paths": []}
    ...
    def _get_function_coverage(self, file_path: str, func_name: str, coverage_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"uncovered_lines": [], "covered_lines": [], "coverage_percentage": 0.0}
    ...
    def _is_edge_case(self, node: ast.AST) -> bool:
        ...
        edge_case_patterns = ["error", "exception", "fail", "invalid", "empty", "null", "none"]
    ...
    async def _generate_test_for_path(self, path_info: dict[str, Any], subtask: Subtask) -> str | None:
    ...
    def _generate_edge_case_test(
        self, module_name: str, func_name: str, test_name: str, path_info: dict[str, Any]
    ) -> str:
...
'''
    ...
    def _parse_test_output(self, output: str) -> dict[str, Any]:
        ...
        if summary_match:
            ...
            results["synthetic_tests_run"] = results["synthetic_tests_passed"] + results["synthetic_tests_failed"]
