...
class BehaviorValidatorAgent(BaseAgent):
        ...
        try:
            if not subtask:
                return self._create_validation_result(
                    subtask_id=task.id, passed=False, issues=["Behavior validation requires a specific subtask"]
                )
    ...
    async def _validate_contract(self, subtask: Subtask) -> dict[str, Any]:
        ...
        issues = []
        warnings = []
        metrics = {"contract_checks": 0, "contract_passed": 0}
    ...
    async def _check_invariants(self, subtask: Subtask) -> dict[str, Any]:
        ...
        issues = []
        warnings = []
        metrics = {"invariant_checks": 0, "invariant_passed": 0}
    ...
    async def _run_pytest(self, args: list[str]) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                result["failed_tests"] = [
                    test.get("nodeid", "") for test in json_report.get("tests", []) if test.get("outcome") == "failed"
                ]
    ...
    def _parse_pytest_output(self, output: str) -> dict[str, Any]:
        ...
        summary_match = re.search(
            r"===\s*(\d+)\s+passed(?:,\s*(\d+)\s+failed)?(?:,\s*(\d+)\s+skipped)?.*?in\s+([\d.]+)s", output
        )
        if summary_match:
