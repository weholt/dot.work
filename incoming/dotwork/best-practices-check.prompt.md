...
- All findings should be appended to the `project-docs/bugs.md` file, with type set to "enhancement" and priority "medium".
...
* **Test Coverage:** Aim for a high coverage percentage (commonly, 80% or more of lines executed by tests). Coverage tools (like pytest-cov with Coverage.py) can report how much of your code is exercised by tests. Be cautious though: 100% coverage doesn’t guarantee bug-free code, but low coverage definitely means parts of your codebase are untested and likely to hide bugs. Focus on covering the core logic and any complex or critical sections of code.
...
Use the following checklist to quickly assess the quality of a Python project or to ensure you haven't missed anything in your own project. Add this to the elements added to the `project-docs/bugs.md` file for easy reference, following the format specified earlier.
