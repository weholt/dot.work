...
def _render_feature(feature: Feature) -> list[str]:
    ...
    args_summary = (
        ", ".join(f"{arg.name}: {arg.annotation or 'unknown'}" for arg in feature.args) or "None"
    )
    ...
    lines.append(
        f"- **Quality:** complexity {feature.avg_complexity}, maintainability {feature.maintainability_index}"
    )
...
def _feature_examples(feature: Feature) -> list[str]:
    ...
    if feature.cli:
        examples.append(
            f"Run the CLI command that triggers `{feature.name}` and capture the console output."
        )
        examples.append("List the required flags and what they influence.")
    if feature.api or feature.web:
        examples.append(
            f"Issue a sample request exercising `{feature.name}` and note the expected response fields."
        )
        examples.append("Record one happy-path payload and the expected status/result.")
    if feature.mcp:
        examples.append(
            f"Describe how an MCP client would call `{feature.name}` and what payload to expect."
        )
